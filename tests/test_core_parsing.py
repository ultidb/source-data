...
class TestParseSeededName:
    def test_single_digit_seed(self):
        assert parse_seeded_name("Vicious Cycle (4)") == ("Vicious Cycle", 4)

    def test_multi_digit_seed(self):
        assert parse_seeded_name("Truck Stop (12)") == ("Truck Stop", 12)

    def test_no_seed(self):
        assert parse_seeded_name("Vicious Cycle") == ("Vicious Cycle", None)

    def test_strips_surrounding_whitespace(self):
        assert parse_seeded_name("  Vicious Cycle (4)  ") == ("Vicious Cycle", 4)

    def test_trailing_paren_no_digits_is_not_a_seed(self):
        # Original inlined logic in parse.py would raise ValueError here;
        # this function must fall back gracefully instead.
        assert parse_seeded_name("Team()") == ("Team()", None)

    def test_trailing_paren_non_numeric_is_not_a_seed(self):
        assert parse_seeded_name("Team (Fugue)") == ("Team (Fugue)", None)

    def test_empty_string(self):
        assert parse_seeded_name("") == ("", None)

    def test_zero_seed(self):
        assert parse_seeded_name("Bye (0)") == ("Bye", 0)

    def test_name_with_slash_untouched(self):
        # Slash-to-dash normalization is convertTeamLinkToTeam's job, not
        # parse_seeded_name's -- this function only strips the seed suffix.
        assert parse_seeded_name("A/B Team (3)") == ("A/B Team", 3)
