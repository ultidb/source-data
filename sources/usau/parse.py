"""USAU HTML parsing: play.usaultimate.org tournament/team/calendar pages ->
models.py domain objects.

Moved verbatim from the old top-level parse.py (MULTI-SOURCE-REDESIGN.md's
"What splits out of parse.py", Phase 3). `extract_nickname`,
`convertNonDigitScore`, `parseCoaches`, and `stagesHaveGames` now live in
`core.parsing` (imported below) instead of being duplicated here -- see that
module's docstring. `convertTeamLinkToTeam`'s seed extraction now goes
...
def convertTeamLinkToTeam(link, teams):
    ...
    if stripped[-1] == ")":
        # parse_seeded_name (core.parsing) replaces the old inline backward
        # scan, which raised ValueError on a malformed name like "Team()";
        # parse_seeded_name falls back to (text, None) instead of crashing,
        # so this case now falls through to "TEAM_NAME_NOT_FOUND" below
        # rather than blowing up the whole scrape.
        parsed_name, parsed_seed = parse_seeded_name(stripped)
